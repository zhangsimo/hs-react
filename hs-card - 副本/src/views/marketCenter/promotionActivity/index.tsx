...
const Theme = () => {
    ...
    const onAdd = () => {
        history.push('/marketCenter/addPromotionActivity')
    }
    ...
    const getTableData = (tableParams, params) => {
        ...
        setLoading(true)
        return api.getActivityThemeList({ ...api.formatParams(tableParams, { ...params, classify: 3 }) }).then(res => {
            setLoading(false)
            return {
                list: res.data.items,
    ...
    const onEdit = row => {
        ...
        const turl = `/marketCenter/addPromotionActivity?id=${row.id}`
        history.push(turl)
    }
    // 点击发布按钮
    const onPublish = row => {
    ...
    const onDataStatistics = row => {
        ...
        history.push('/marketCenter/promotionStatistics?' + seach)
    }
    ...
    const onViewDetails = row => {
        ...
        history.push('/marketCenter/promotionDetails?id=' + row.id)
    }
        ...
        },
        {
            ...
            render: (row: any) => (
                <div>
                    ...
                    {(+row.issueStatus !== 0 && +[6].includes(+row.type)) ? (<Button onClick={() => onDataStatistics(row)} type="link">数据统计</Button>) : null}
