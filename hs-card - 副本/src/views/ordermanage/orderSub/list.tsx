...
const WorkList: React.FC<IProps> = props => {
  ...
  const getTableData = (tableParams, params) =>
    //getOrderProFindPage
    api
      .getOrderProFindPage({ ...api.formatParams(tableParams, params), ...{ orderChannel: props.orderType, startTime: form.getFieldValue('startTime'), endTime: form.getFieldValue('endTime') } })
      .then(res => {
        setLoading(false)
        return {
          list: res.data.items,
