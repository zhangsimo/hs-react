...
import {Form, Input, Button, Table, message, Select, InputNumber} from 'antd'
...
let StandardItem: React.FC<IProps> = props => {
  ...
  const getTableData = (tableParams, params) =>
    api.getOrderGoodQueryItemList({...api.formatParams(tableParams, params), ...{status: 2, detail: 1}}).then(res => {
  ...
  const columns: ColumnProps<any>[] = [
    ...
    },
    {
      ...
      width: 60,
      align: 'center',
      render: (value, row) => <InputNumber min={1} max={100000} defaultValue={1} onChange={onChange} />,
    },
  ]
